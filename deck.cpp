...
void deck::insert(const std::string& filename)
{
    ...
    if(cards.is_open())
    {
        ...
        while(!cards.eof())
        {
            cards >> mana >> damage >> health >> taunt >> charge >> shield >> windfury >> stealth;
            std::getline(cards, name);
            trim(name);

            card c(name, mana, damage, health, (taunt > 0 ? true : false), (charge > 0 ? true : false), (shield > 0 ? true : false), (windfury > 0 ? true : false), (stealth > 0 ? true : false));
